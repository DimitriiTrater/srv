...
void print_progress(int current, int total) {
    ...
    {
        size_t position = static_cast<size_t>((bar_width - 2) * static_cast<float>(i) / (name_length - 1));
        letter_positions.push_back(position);
    }
    ...
    for (size_t i = 0; i < name_length; ++i)
    {
        if (letter_positions[i] < pos)
        {
            bar[letter_positions[i]] = name[i];
        }
    }
